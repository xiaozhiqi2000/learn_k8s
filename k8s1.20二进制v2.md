...
# 分发证书
for node in node1 node2 node3;do scp -f \
 /root/k8s/ssl/kube-apiserver*.pem \
 /root/k8s/ssl/front-proxy-client*.pem \
 /root/k8s/ssl/sa*
$node:/etc/kubernetes/pki/;done
```

#### 3.6. 生成serviceaccout使用的私钥和公钥
```shell
