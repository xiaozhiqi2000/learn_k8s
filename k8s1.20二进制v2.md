...
```shell
# 创建ssl文件夹
mkdir -p /root/k8s/ssl/ /root/manifests/ && cd /root/k8s/ssl/
...
> api 的客户端证书聚合 https://kubernetes.io/zh/docs/tasks/extend-kubernetes/setup-extension-api-server/
...
# 分发证书
for node in node1 node2 node3;do scp -f /root/k8s/ssl/kube-apiserver*.pem /root/k8s/ssl/front-proxy-client*.pem $node:/etc/kubernetes/pki/;done
```
...
BOOTSTRAP_TOKEN=$(cat bootstrap-secret.yaml | sed -n '/token-.*:/s#.*: ##p' | sed "N;s#\n#\.#")
...
```shell
# 生成证书
cd /root/k8s/ssl/ && cfssl gencert --ca ca.pem --ca-key ca-key.pem --config ca-config.json --profile client kube-proxy-csr.json | cfssljson --bare kube-proxy
...
# 下载flannel yaml
wget -O flannel.yml https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml
...
# 安装Dashboad
curl https://raw.githubusercontent.com/kubernetes/dashboard/v2.2.0/aio/deploy/recommended.yaml -o dashboad.yaml
