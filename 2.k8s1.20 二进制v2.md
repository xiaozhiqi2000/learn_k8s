...
```shell
cat > /root/k8s/ssl/kubernetes-admin-csr.json <<EOF
{
    "CN": "kubernetes-admin",
    "key": {
        "algo": "rsa",
...
```shell
cd /root/k8s/ssl/ && \
cfssl gencert --ca ca.pem --ca-key ca-key.pem --config ca-config.json --profile client kubernetes-admin-csr.json | cfssljson --bare kubernetes-admin
